...
test('Gateway restart does not resend an uncertain reply and preserves queued work', async t => {
    ...
    t.mock.method(globalThis, 'fetch', async (_url: any, options: any) => {
        sent++; payloads.push(JSON.parse(options.body));
        if (sent === 1) storageFailed = true;
        return Response.json({ id: 'reply' });
    });
    const bot = new DiscordGateway(context as any, { DISCORD_BOT_TOKEN: 'token', DISCORD_CHANNEL_ID: channelId, APP_PASSWORD: 'a-long-test-password' } as Env);
    await initialized!;
    await assert.rejects(bot.alarm());
    assert.equal(sent, 1);
    assert.ok(stored.queue[0].sendingAt);
    storageFailed = false;
    const restarted = new DiscordGateway(context as any, { DISCORD_BOT_TOKEN: 'token', DISCORD_CHANNEL_ID: channelId, APP_PASSWORD: 'a-long-test-password' } as Env);
    await initialized!;
...
test('Gateway persists entire batches and resumes after rate limits with independent image failures', async t => {
    ...
    const env = {
        ...
        RANK_BROWSER: { getByName: () => ({ fetch: async () => {
            lookups++;
            return lookups === 2 ? new Response(null, { status: 500 }) : Response.json({ content: `result ${lookups}` });
        } }) },
    } as unknown as Env;
